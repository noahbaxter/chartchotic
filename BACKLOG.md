...
- **Preview sync drift when host MIDI track is closed** ([#24](https://github.com/noahbaxter/chartchotic/issues/24)) — Reported by Invontion (REAPER 7.68, Chartchotic 1.2.2): visuals play earlier than audio when the MIDI track Chartchotic is loaded on isn't open in REAPER's MIDI editor. Opening that track restores sync. Track discovery / calibration settings don't matter. Likely cause: REAPER serves stale `MIDI_GetProjQNFromPPQPos` conversions (`ReaperNoteFetcher.cpp:215-216`) when the take's MIDI editor isn't open, particularly around tempo changes. Different from a constant offset (calibration would fix that) — drift that depends on tempo map. Needs REAPER repro session to diagnose. Workaround: open the host track's MIDI editor.
